...
if [ "$INPUT_SOURCE_REPO" != "$INPUT_DESTINATION_REPO" ]; then 
  ...
  CREATION_PREFIX="${INPUT_SOURCE_OWNER}:"
  git fetch "$INPUT_SOURCE_REPO" "${SOURCE_BRANCH}:${INPUT_SOURCE_OWNER}/${SOURCE_BRANCH}"
else
  VALIDATION_PREFIX=""
  CREATION_PREFIX=""
fi

if [ "$(git rev-parse --revs-only "${VALIDATION_PREFIX}${SOURCE_BRANCH}")" = "$(git rev-parse --revs-only "$DESTINATION_BRANCH")" ]; then 
...
LINES_CHANGED=$(git diff --name-only "$DESTINATION_BRANCH" "${VALIDATION_PREFIX}${SOURCE_BRANCH}" | wc -l | awk '{print $1}')
...
COMMAND="hub pull-request \
  ...
  -h ${CREATION_PREFIX}${SOURCE_BRANCH} \
  --no-edit \
  $PR_ARG \
  || true"
